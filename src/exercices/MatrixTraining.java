...
public class MatrixTraining {
    ...
    public int[][] intMatrix(int[] a, int[] b, int[] c) {
        int[][] ret = new int[3][Math.max(Math.max(a.length, b.length), c.length)];
        for (int i=0; i<a.length; i++) { ret[0][i] = a[i]; }
        for (int i=0; i<b.length; i++) { ret[1][i] = b[i]; }
        for (int i=0; i<c.length; i++) { ret[2][i] = c[i]; }
        return ret ;
    }
    ...
    public String[][] stringMatrix(String[] a, String[] b, String[] c) {
        String[][] ret = new String[3][Math.max(Math.max(a.length, b.length), c.length)];
        for (int i=0; i<a.length; i++) { ret[0][i] = a[i]; }
        for (int i=0; i<b.length; i++) { ret[1][i] = b[i]; }
        for (int i=0; i<c.length; i++) { ret[2][i] = c[i]; }
        return ret ;
    ...
    public boolean contains(int[][] matrix, int search) {
        ...
        for (int i = 0; i < matrix.length; i++) {
            for (int j=0; j<matrix[0].length; j++) {
                if ( matrix[i][j] == search) {trouve = true;}
            }
        }
        return trouve;
    }
